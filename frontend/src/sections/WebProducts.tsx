...
export const WebProducts = () => (
  <section id="web-products" aria-labelledby="web-products-heading" className="scroll-mt-24 border-t border-[color:var(--arva-border)] bg-[#111111]">
    <div className="mx-auto w-full max-w-site px-4 py-20 sm:px-6 sm:py-24 lg:px-8 lg:py-28">
      ...
      <Reveal delay={140}>
        <div className="mt-14 hidden overflow-hidden rounded-none border border-[color:var(--arva-border)] md:block">
      ...
      <div className="mt-14 grid gap-4 md:hidden" data-testid="web-products-pricing-cards">
        {ROWS.map((r, i) => (
          <Reveal key={r.product} delay={i * 50}>
            <div className="rounded-none border border-[color:var(--arva-border)] bg-[color:var(--arva-surface)] p-5">
      ...
      <Reveal delay={180}>
        <div className="mt-12">
          ...
          <ul className="mt-4 flex flex-wrap gap-2" data-testid="web-products-built-with">
            {BUILT_WITH.map((t) => (
              <li key={t} className="inline-flex items-center rounded-none border border-[color:var(--arva-border)] px-3 py-1.5 text-xs text-[color:var(--arva-text-muted)] font-mono uppercase">
                {t}
              </li>
            ))}
          </ul>
